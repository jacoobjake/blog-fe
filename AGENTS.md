...
### API conventions
- **Reads use GraphQL** (`POST /graphql`) for all modules — blogs, authors, assets, auth (`me`), users, tags, etc.
- **Writes use REST** under `/api/admin/*` and `/api/assets`.
- New modules should follow the same split: GraphQL queries for fetch/list/detail, REST for mutations.
